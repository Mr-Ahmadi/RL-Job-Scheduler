...
import numpy as np
import re
from nero.envs.problem import ProblemDescription, model_names, batch_sizes_range

#: A GPU slot runs at most this many co-located jobs.  Every feasibility check in
#: the project goes through ``feasible_slots`` / ``feasible_mask`` below, so this
#: rule is stated once.
MAX_JOBS_PER_GPU = 2
...
class Base_JobSchedulingEnv(gym.Env):
    ...
    def slot_occupancy(self):
        """``(s, a) -> [job indices]`` for every occupied slot."""
        occ = {}
        for j, s, a in np.argwhere(self.assignment > 0):
            occ.setdefault((int(s), int(a)), []).append(int(j))
        return occ

    def feasible_slots(self, j=None, occ=None):
        """Slots job ``j`` may be placed on: those holding fewer than two jobs.

        The single source of truth for feasibility -- the action masks, the
        baselines, the deployment actors and the top-K search all call this, so
        they cannot drift apart.
        """
        if j is None:
            j = self.current_job_idx
        if j >= self.J or self.assignment[j].sum() > 0:
            return []
        if occ is None:
            occ = self.slot_occupancy()
        return [(s, a)
                for s in range(self.S) for a in range(self.A)
                if len([oj for oj in occ.get((s, a), ()) if oj != j]) < MAX_JOBS_PER_GPU]

    def feasible_mask(self, j=None, occ=None):
        """``feasible_slots`` as a flat 0/1 mask over ``s * A + a``."""
        mask = np.zeros(self.S * self.A, dtype=np.float32)
        for (s, a) in self.feasible_slots(j, occ=occ):
            mask[s * self.A + a] = 1.0
        return mask

    def _has_available_resources(self):
        return bool(np.any(self.assignment.sum(axis=0) < MAX_JOBS_PER_GPU))
