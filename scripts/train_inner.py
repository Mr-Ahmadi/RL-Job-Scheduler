...
def get_valid_action_indices(env):
    """Indices of the feasible slots for the current job."""
    return [s * env.A + a for (s, a) in env.feasible_slots()]
...
if __name__ == "__main__":
    ...
    with open(str(SCORES / "training_ppo.json"), 'w') as f:
    ...
    with open(str(SCORES / "evaluation_scores_ppo.json"), 'w') as f:
        json.dump(rewards, f)
